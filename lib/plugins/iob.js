...
function init(ctx) {
  ...
  function googleHomeIOBIntentHandler(result, next, sbx) {
    var preamble = result && result.parameters && result.parameters.givenName ? result.parameters.givenName + translate(' has ') : translate('You have');
    var message = preamble + getIob(sbx) + translate('insulin on board');
    next(message);
  }
