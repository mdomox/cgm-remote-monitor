...
function configure (app, wares, ctx, env) {
    ...
    function handleIntent(body, next) {
      ...
      var metric = body.queryResult.parameters ? body.queryResult.parameters.metric : null;
        var languageCode = body.queryResult.intent.languageCode;
        if(languageCode){
            if(languageCode.length > 2) {
                languageCode = languageCode.substr(0, 2);
            }
            ctx.language.set(languageCode);
            moment.languageCode(languageCode);
        }
      var handler = ctx.googleHome.getIntentHandler(displayName, metric, languageCode);
      if (handler) {
