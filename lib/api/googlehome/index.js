...
function configure (app, wares, ctx, env) {
    ...
    ctx.plugins.eachEnabledPlugin(function each(plugin){
        if (plugin.googlehome) {
            if (plugin.googlehome.intentHandlers) {
                console.log(plugin.name + ' is GoogleHome enabled');
                _each(plugin.googlehome.intentHandlers, function (route) {
                    if (route) {
                        ctx.googlehome.configureIntentHandler(route.intent, route.intentHandler, route.routableSlot, route.slots);
                    }
                });
            }
            if (plugin.googlehome.rollupHandlers) {
                console.log(plugin.name + ' is GoogleHome rollup enabled');
                _each(plugin.googlehome.rollupHandlers, function (route) {
                    console.log('Route');
                    console.log(route);
                    if (route) {
            ...
            console.log('Plugin ' + plugin.name + ' is not GoogleHome enabled');
        }
    });

    api.post('/googlehome', ctx.authorization.isPermitted('api:*:read'), function (req, res, next) {
	console.log('Incoming request from GoogleHome');        
	var locale = req.body.request.languageCode;
        if(locale){
            if(locale.length > 2) {
        ...
        switch (req.body.request.type) {
            case 'IntentRequest':
                onIntent(req.body.request.intent, function (title, response) {
                    res.json(ctx.googlehome.buildSpeechletResponse(title, response, '', 'true'));
                    next( );
                });
                break;
            case 'LaunchRequest':
            ...
            case 'SessionEndedRequest':
                onSessionEnded(req.body.request.intent, function (googlehomeResponse) {
                    res.json(googlehomeResponse);
                    next( );
                });
                break;
        }
    ...
    ctx.googlehome.addToRollup('Status', function bgRollupHandler(slots, sbx, callback) {
        entries.list({count: 1}, function (err, records) {
            ...
            var status = translate('googlehomeStatus', {
                params: [
    ...
    function handleIntent(intentName, slots, next) {
        var handler = ctx.googlehome.getIntentHandler(intentName, slots);
        if (handler){
